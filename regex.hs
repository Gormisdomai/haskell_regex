...
inferRegexV0 match reject = head $ (filter (\r -> and (getMatches r match) && ((not.or)(getMatches r reject)))) (bottomUpRegexBuilder ((nub.concat)(match ++ reject))) where
  getMatches r tests = map (evalNFA ((compileNFA.buildNFA) r)) tests
